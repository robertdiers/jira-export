...
public class ExcelXmlExportHelper {
	...
	public static void writeXmlHeader(StringWriter buf, int worksheetcount) {
		...
		buf.write(ExcelXmlExportHelper.line_separator);
		buf.append("   <Alignment ss:Horizontal=\"Left\" ss:Vertical=\"Top\" ss:WrapText=\"1\"/>");
		buf.write(ExcelXmlExportHelper.line_separator);
	...
	public static void writeXmlWorksheetHeader(StringWriter buf, String sheetname, List<String> headlines, int maxrows) {
		...
		for (String headline : headlines) {
			double width = 0.0;
			if (headline.contains("CHANGE") || headline.contains("COMMENT")){
				width = 5.25 * (headline.length()+2) + 170; //added filter dropdown
			}else {
				width = 5.25 * (headline.length()+2) + 50; //added filter dropdown
			}
	 ...
	 * @param colorcount
	 */
	public static void writeXmlRowHeader(StringWriter buf, long elements_total) {
		double height = 15.0;
		if(elements_total>0){
			height=height * elements_total+15;
		}
		buf.append("   <Row ss:AutoFitHeight=\"0\" ss:Height=\"" + height + "\">");
		buf.write(ExcelXmlExportHelper.line_separator);
	}
